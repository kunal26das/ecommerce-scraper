...
query_refrigerators = "/search?sid=j9e%2Fabm%2Fhzg&otracker=CLP_Filters"
...
def getRefrigeratorBrands():
    brands = []
    with open("flipkart_brands_refrigerators.txt", "r") as infile:
        for brand in infile:
...
def getPageLimit(brand):
    url = url_flipkart + query_refrigerators + query_brand + brand
    driver.get(url)
...
def getRefrigerators():
    count = 0
    refrigerators = []
    uniqueID = getUniqueID()
    brands = getRefrigeratorBrands()
    csvfile = open("flipkart_refrigerators.csv", "w")
    jsonfile = open("flipkart_refrigerators.json", "w")
    for brand in brands:
        page_limit = getPageLimit(brand)
        for page in range(1, page_limit+1):
            url = url_flipkart + query_refrigerators + query_brand + brand + query_page + str(page)
            driver.get(url)
            ...
            for link in links:
                ...
                try:
                    ...
                    continue
    json.dump({"records": refrigerators}, jsonfile)
