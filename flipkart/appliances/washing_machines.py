...
query_washing_machines = "/search?sid=j9e%2Fabm%2F8qx&otracker=CLP_Filters"
...
def getWashingMachineBrands():
    brands = []
    with open("flipkart_brands_washing_machines.txt", "r") as infile:
        for brand in infile:
...
def getPageLimit(brand):
    url = url_flipkart + query_washing_machines + query_brand + brand
    driver.get(url)
...
def getWashingMachines():
    count = 0
    washing_machines = []
    uniqueID = getUniqueID()
    brands = getWashingMachineBrands()
    csvfile = open("flipkart_washing_machines.csv", "w")
    jsonfile = open("flipkart_washing_machines.json", "w")
    for brand in brands:
        page_limit = getPageLimit(brand)
        for page in range(1, page_limit+1):
            url = url_flipkart + query_washing_machines + query_brand + brand + query_page + str(page)
            driver.get(url)
            ...
            for link in links:
                ...
                try:
                    ...
                    continue
    json.dump({"records": washing_machines}, jsonfile)
