...
query_groceries = "/grocery/personal-baby-care/diapers-wipes/pr?sid=73z,njl,smb&marketplace=GROCERY"
query_brand = "&p%5B%5D=facets.brand%255B%255D%3D"
query_page = "&page="
category = "diapers_wipes"
...
def getGroceryBrands():
    brands = []
    with open("flipkart_brands_personal_baby_care_diapers_wipes.txt", "r") as infile:
        for brand in infile:
...
def getGroceries():
    ...
    csvfile = open("flipkart_personal_baby_care_diapers_wipes.csv", "w")
    jsonfile = open("flipkart_personal_baby_care_diapers_wipes.json", "w")
    for brand in brands:
