...
query_groceries = "/grocery/personal-baby-care/shaving-needs/pr?sid=73z,njl,nw3&marketplace=GROCERY"
query_brand = "&p%5B%5D=facets.brand%255B%255D%3D"
query_page = "&page="
category = "shaving_needs"
...
def getGroceryBrands():
    brands = []
    with open("flipkart_brands_personal_baby_care_shaving_needs.txt", "r") as infile:
        for brand in infile:
...
def getGroceries():
    ...
    csvfile = open("flipkart_personal_baby_care_shaving_needs.csv", "w")
    jsonfile = open("flipkart_personal_baby_care_shaving_needs.json", "w")
    for brand in brands:
