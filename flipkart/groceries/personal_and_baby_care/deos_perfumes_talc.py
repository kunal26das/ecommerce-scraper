...
query_groceries = "/grocery/personal-baby-care/deos-perfumes-talc/pr?sid=73z,njl,np3&marketplace=GROCERY"
query_brand = "&p%5B%5D=facets.brand%255B%255D%3D"
query_page = "&page="
category = "deos_perfumes_talc"
...
def getGroceryBrands():
    brands = []
    with open("flipkart_brands_personal_baby_care_deos_perfumes_talc.txt", "r") as infile:
        for brand in infile:
...
def getGroceries():
    ...
    csvfile = open("flipkart_personal_baby_care_deos_perfumes_talc.csv", "w")
    jsonfile = open("flipkart_personal_baby_care_deos_perfumes_talc.json", "w")
    for brand in brands:
