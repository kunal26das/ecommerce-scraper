...
query_groceries = "/grocery/personal-baby-care/kajal-makeup/pr?sid=73z,njl,lzq&marketplace=GROCERY"
query_brand = "&p%5B%5D=facets.brand%255B%255D%3D"
query_page = "&page="
category = "kajal_makeup"
...
def getGroceryBrands():
    brands = []
    with open("flipkart_brands_personal_baby_care_kajal_makeup.txt", "r") as infile:
        for brand in infile:
...
def getGroceries():
    ...
    csvfile = open("flipkart_personal_baby_care_kajal_makeup.csv", "w")
    jsonfile = open("flipkart_personal_baby_care_kajal_makeup.json", "w")
    for brand in brands:
