...
query_groceries = "/grocery/personal-baby-care/sanitary-needs/pr?sid=73z,njl,tlj&marketplace=GROCERY"
query_brand = "&p%5B%5D=facets.brand%255B%255D%3D"
query_page = "&page="
category = "sanitary_needs"
...
def getGroceryBrands():
    brands = []
    with open("flipkart_brands_personal_baby_care_sanitary_needs.txt", "r") as infile:
        for brand in infile:
...
def getGroceries():
    ...
    csvfile = open("flipkart_personal_baby_care_sanitary_needs.csv", "w")
    jsonfile = open("flipkart_personal_baby_care_sanitary_needs.json", "w")
    for brand in brands:
