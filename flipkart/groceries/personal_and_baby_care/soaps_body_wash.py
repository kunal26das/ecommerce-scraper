...
query_groceries = "/grocery/personal-baby-care/soaps-body-wash/pr?sid=73z,njl,sn6&marketplace=GROCERY"
query_brand = "&p%5B%5D=facets.brand%255B%255D%3D"
query_page = "&page="
category = "soaps_body_wash"
...
def getGroceryBrands():
    brands = []
    with open("flipkart_brands_personal_baby_care_soaps_body_wash.txt", "r") as infile:
        for brand in infile:
...
def getGroceries():
    ...
    csvfile = open("flipkart_personal_baby_care_soaps_body_wash.csv", "w")
    jsonfile = open("flipkart_personal_baby_care_soaps_body_wash.json", "w")
    for brand in brands:
