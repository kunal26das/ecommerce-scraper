...
query_groceries = "/grocery/staples/masalas-spices/pr?sid=73z,bpe,a6m&marketplace=GROCERY"
query_brand = "&p%5B%5D=facets.brand%255B%255D%3D"
query_page = "&page="
category = "masalas_spices"
...
def getGroceryBrands():
    brands = []
    with open("flipkart_brands_staples_masalas_spices.txt", "r") as infile:
        for brand in infile:
...
def getGroceries():
    ...
    csvfile = open("flipkart_brands_staples_masalas_spices.csv", "w")
    jsonfile = open("flipkart_brands_staples_masalas_spices.json", "w")
    for brand in brands:
