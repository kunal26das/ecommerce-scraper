...
query_groceries = "/grocery/staples/atta-flours/pr?sid=73z,bpe,9da&marketplace=GROCERY"
query_brand = "&p%5B%5D=facets.brand%255B%255D%3D"
query_page = "&page="
category = "atta_flours"
...
def getGroceryBrands():
    brands = []
    with open("flipkart_brands_staples_atta_flours.txt", "r") as infile:
        for brand in infile:
...
def getGroceries():
    ...
    csvfile = open("flipkart_brands_staples_atta_flours.csv", "w")
    jsonfile = open("flipkart_brands_staples_atta_flours.json", "w")
    for brand in brands:
