...
query_groceries = "/grocery/staples/dry-fruits-nuts-seeds/pr?sid=73z,bpe,dtp&marketplace=GROCERY"
query_brand = "&p%5B%5D=facets.brand%255B%255D%3D"
query_page = "&page="
category = "dry_fruits_nuts_seeds"
...
def getGroceryBrands():
    brands = []
    with open("flipkart_brands_staples_dry_fruits_nuts_seeds.txt", "r") as infile:
        for brand in infile:
...
def getGroceries():
    ...
    csvfile = open("flipkart_brands_staples_dry_fruits_nuts_seeds.csv", "w")
    jsonfile = open("flipkart_brands_staples_dry_fruits_nuts_seeds.json", "w")
    for brand in brands:
