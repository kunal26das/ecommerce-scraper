...
query_groceries = "/grocery/staples/rice-rice-products/pr?sid=73z,bpe,zwp&marketplace=GROCERY"
query_brand = "&p%5B%5D=facets.brand%255B%255D%3D"
query_page = "&page="
category = "rice_rice_products"
...
def getGroceryBrands():
    brands = []
    with open("flipkart_brands_staples_rice_rice_products.txt", "r") as infile:
        for brand in infile:
...
def getGroceries():
    ...
    csvfile = open("flipkart_brands_staples_rice_rice_products.csv", "w")
    jsonfile = open("flipkart_brands_staples_rice_rice_products.json", "w")
    for brand in brands:
