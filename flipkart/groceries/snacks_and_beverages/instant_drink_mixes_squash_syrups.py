...
query_groceries = "/grocery/snacks-beverages/instant-drink-mixes-squash-syrups/pr?sid=73z,ujs,iau&marketplace=GROCERY"
query_brand = "&p%5B%5D=facets.brand%255B%255D%3D"
query_page = "&page="
category = "instant_drink_mixes_squash_syrups"
...
def getGroceryBrands():
    brands = []
    with open("flipkart_brands_snacks_beverages_instant_drink_mixes_squash_syrups.txt", "r") as infile:
        for brand in infile:
...
def getGroceries():
    ...
    csvfile = open("flipkart_brands_snacks_beverages_instant_drink_mixes_squash_syrups.csv", "w")
    jsonfile = open("flipkart_brands_snacks_beverages_instant_drink_mixes_squash_syrups.json", "w")
    for brand in brands:
