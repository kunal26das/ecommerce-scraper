...
query_groceries = "/grocery/snacks-beverages/chips-namkeen-snacks/pr?sid=73z,ujs,dd9&marketplace=GROCERY"
query_brand = "&p%5B%5D=facets.brand%255B%255D%3D"
query_page = "&page="
category = "chips_namkeen_snacks"
...
def getGroceryBrands():
    brands = []
    with open("flipkart_brands_snacks_beverages_chips_namkeen_snacks.txt", "r") as infile:
        for brand in infile:
...
def getGroceries():
    ...
    csvfile = open("flipkart_brands_snacks_beverages_chips_namkeen_snacks.csv", "w")
    jsonfile = open("flipkart_brands_snacks_beverages_chips_namkeen_snacks.json", "w")
    for brand in brands:
