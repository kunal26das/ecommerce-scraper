...
query_groceries = "/grocery/packaged-food/ketchups-spreads/pr?sid=73z,u0u,0tl&marketplace=GROCERY"
query_brand = "&p%5B%5D=facets.brand%255B%255D%3D"
query_page = "&page="
category = "ketchups_spreads"
...
def getGroceryBrands():
    brands = []
    with open("flipkart_brands_packaged_food_ketchups_spreads.txt", "r") as infile:
        for brand in infile:
...
def getGroceries():
    ...
    csvfile = open("flipkart_brands_packaged_food_ketchups_spreads.csv", "w")
    jsonfile = open("flipkart_brands_packaged_food_ketchups_spreads.json", "w")
    for brand in brands:
