...
query_groceries = "/grocery/packaged-food/noodles-pasta/pr?sid=73z,u0u,ltz&marketplace=GROCERY"
...
def getGroceryBrands():
    brands = []
    with open("flipkart_brands_packaged_food_noodles_pasta.txt", "r") as infile:
        for brand in infile:
...
def getGroceries():
    ...
    csvfile = open("flipkart_brands_packaged_food_noodles_pasta.csv", "w")
    jsonfile = open("flipkart_brands_packaged_food_noodles_pasta.json", "w")
    for brand in brands:
