...
query_groceries = "/grocery/packaged-food/cooking-sauces-vinegar/pr?sid=73z,u0u,wd7&marketplace=GROCERY"
query_brand = "&p%5B%5D=facets.brand%255B%255D%3D"
query_page = "&page="
category = "cooking_sauces_vinegar"
...
def getGroceryBrands():
    brands = []
    with open("flipkart_brands_packaged_food_cooking_sauces_vinegar.txt", "r") as infile:
        for brand in infile:
...
def getGroceries():
    ...
    csvfile = open("flipkart_brands_packaged_food_cooking_sauces_vinegar.csv", "w")
    jsonfile = open("flipkart_brands_packaged_food_cooking_sauces_vinegar.json", "w")
    for brand in brands:
