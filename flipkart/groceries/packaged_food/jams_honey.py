...
query_groceries = "/grocery/packaged-food/jams-honey/pr?sid=73z,u0u,j4e&marketplace=GROCERY"
...
def getGroceryBrands():
    brands = []
    with open("flipkart_brands_packaged_food_jams_honey.txt", "r") as infile:
        for brand in infile:
...
def getGroceries():
    ...
    csvfile = open("flipkart_brands_packaged_food_jams_honey.csv", "w")
    jsonfile = open("flipkart_brands_packaged_food_jams_honey.json", "w")
    for brand in brands:
