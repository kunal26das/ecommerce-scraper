...
query_groceries = "/grocery/packaged-food/ready-to-cook/pr?sid=73z,u0u,0gv&marketplace=GROCERY"
...
def getGroceryBrands():
    brands = []
    with open("flipkart_brands_packaged_food_ready_to_cook.txt", "r") as infile:
        for brand in infile:
...
def getGroceries():
    ...
    csvfile = open("flipkart_brands_packaged_food_ready_to_cook.csv", "w")
    jsonfile = open("flipkart_brands_packaged_food_ready_to_cook.json", "w")
    for brand in brands:
