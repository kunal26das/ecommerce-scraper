...
query_groceries = "/grocery/packaged-food/pickles-chutney/pr?sid=73z,u0u,03x&marketplace=GROCERY"
query_brand = "&p%5B%5D=facets.brand%255B%255D%3D"
query_page = "&page="
category = "pickles_chutney"
...
def getGroceryBrands():
    brands = []
    with open("flipkart_brands_packaged_food_pickles_chutney.txt", "r") as infile:
        for brand in infile:
...
def getGroceries():
    ...
    csvfile = open("flipkart_brands_packaged_food_pickles_chutney.csv", "w")
    jsonfile = open("flipkart_brands_packaged_food_pickles_chutney.json", "w")
    for brand in brands:
