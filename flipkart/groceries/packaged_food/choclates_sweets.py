...
query_groceries = "/grocery/packaged-food/chocolates-sweets/pr?sid=73z,u0u,7o6&marketplace=GROCERY"
query_brand = "&p%5B%5D=facets.brand%255B%255D%3D"
query_page = "&page="
category = "chocolates_sweets"
...
def getGroceryBrands():
    brands = []
    with open("flipkart_brands_packaged_food_chocolates_sweets.txt", "r") as infile:
        for brand in infile:
...
def getGroceries():
    ...
    csvfile = open("flipkart_brands_packaged_food_chocolates_sweets.csv", "w")
    jsonfile = open("flipkart_brands_packaged_food_chocolates_sweets.json", "w")
    for brand in brands:
