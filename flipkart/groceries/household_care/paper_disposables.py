...
query_groceries = "/grocery/household-care/paper-disposables/pr?sid=73z,cwl,2wc&marketplace=GROCERY"
query_brand = "&p%5B%5D=facets.brand%255B%255D%3D"
query_page = "&page="
category = "paper_disposables"
...
def getGroceryBrands():
    brands = []
    with open("flipkart_brands_household_care_paper_disposables.txt", "r") as infile:
        for brand in infile:
...
def getGroceries():
    ...
    csvfile = open("flipkart_household_care_paper_disposables.csv", "w")
    jsonfile = open("flipkart_household_care_paper_disposables.json", "w")
    for brand in brands:
