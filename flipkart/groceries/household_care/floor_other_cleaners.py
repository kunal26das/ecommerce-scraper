...
query_groceries = "/grocery/household-care/floor-other-cleaners/pr?sid=73z,cwl,u3c&marketplace=GROCERY"
query_brand = "&p%5B%5D=facets.brand%255B%255D%3D"
query_page = "&page="
category = "floor_other_cleaners"
...
def getGroceryBrands():
    brands = []
    with open("flipkart_brands_household_care_floor_other_cleaners.txt", "r") as infile:
        for brand in infile:
...
def getGroceries():
    ...
    csvfile = open("flipkart_household_care_floor_other_cleaners.csv", "w")
    jsonfile = open("flipkart_household_care_floor_other_cleaners.json", "w")
    for brand in brands:
