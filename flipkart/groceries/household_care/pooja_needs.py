...
query_groceries = "/grocery/household-care/pooja-needs/pr?sid=73z,cwl,u64&marketplace=GROCERY"
query_brand = "&p%5B%5D=facets.brand%255B%255D%3D"
query_page = "&page="
category = "pooja_needs"
...
def getGroceryBrands():
    brands = []
    with open("flipkart_brands_household_care_pooja_needs.txt", "r") as infile:
        for brand in infile:
...
def getGroceries():
    ...
    csvfile = open("flipkart_household_care_pooja_needs.csv", "w")
    jsonfile = open("flipkart_household_care_pooja_needs.json", "w")
    for brand in brands:
