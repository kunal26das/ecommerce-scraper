...
query_groceries = "/grocery/household-care/utensil-cleaners/pr?sid=73z,cwl,bdc&marketplace=GROCERY"
query_brand = "&p%5B%5D=facets.brand%255B%255D%3D"
query_page = "&page="
category = "utensil_cleaners"
...
def getGroceryBrands():
    brands = []
    with open("flipkart_brands_household_care_utensil_cleaners.txt", "r") as infile:
        for brand in infile:
...
def getGroceries():
    ...
    csvfile = open("flipkart_household_care_utensil_cleaners.csv", "w")
    jsonfile = open("flipkart_household_care_utensil_cleaners.json", "w")
    for brand in brands:
