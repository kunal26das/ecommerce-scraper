...
query_groceries = "/grocery/household-care/shoe-care/pr?sid=73z,cwl,5mx&marketplace=GROCERY"
...
def getGroceryBrands():
    brands = []
    with open("flipkart_brands_household_care_shoe_care.txt", "r") as infile:
        for brand in infile:
...
def getGroceries():
    ...
    csvfile = open("flipkart_household_care_shoe_care.csv", "w")
    jsonfile = open("flipkart_household_care_shoe_care.json", "w")
    for brand in brands:
