...
query_groceries = "/grocery/household-care/repellants-fresheners/pr?sid=73z,cwl,qz9&marketplace=GROCERY"
query_brand = "&p%5B%5D=facets.brand%255B%255D%3D"
query_page = "&page="
category = "repellants_fresheners"
...
def getGroceryBrands():
    brands = []
    with open("flipkart_brands_household_care_repellants_fresheners.txt", "r") as infile:
        for brand in infile:
...
def getGroceries():
    ...
    csvfile = open("flipkart_household_care_repellants_fresheners.csv", "w")
    jsonfile = open("flipkart_household_care_repellants_fresheners.json", "w")
    for brand in brands:
