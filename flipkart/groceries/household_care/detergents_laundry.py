...
query_groceries = "/grocery/household-care/detergents-laundry/pr?sid=73z,cwl,2z2&marketplace=GROCERY"
query_brand = "&p%5B%5D=facets.brand%255B%255D%3D"
query_page = "&page="
category = "detergents_laundry"
...
def getGroceryBrands():
    brands = []
    with open("flipkart_brands_household_care_detergents_laundry.txt", "r") as infile:
        for brand in infile:
...
def getGroceries():
    ...
    csvfile = open("flipkart_household_care_detergents_laundry.csv", "w")
    jsonfile = open("flipkart_household_care_detergents_laundry.json", "w")
    for brand in brands:
