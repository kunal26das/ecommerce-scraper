...
query_groceries = "/grocery/household-care/pet-food/pr?sid=73z,cwl,m92&marketplace=GROCERY"
...
def getGroceryBrands():
    brands = []
    with open("flipkart_brands_household_care_pet_food.txt", "r") as infile:
        for brand in infile:
...
def getGroceries():
    ...
    csvfile = open("flipkart_household_care_pet_food.csv", "w")
    jsonfile = open("flipkart_household_care_pet_food.json", "w")
    for brand in brands:
