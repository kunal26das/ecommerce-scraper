...
query_laptops = "/search?sid=6bo%2Cb5g&otracker=CLP_Filters"
...
def getLaptopBrands():
    brands = []
    with open("flipkart_brands_laptops.txt", "r") as infile:
        for brand in infile:
...
def getPageLimit(brand):
    url = url_flipkart + query_laptops + query_brand + brand
    driver.get(url)
...
def getLaptops():
    ...
    brands = getLaptopBrands()
    csvfile = open("flipkart_laptops.csv", "w")
    jsonfile = open("flipkart_laptops.json", "w")
    for brand in brands:
        page_limit = getPageLimit(brand)
        for page in range(1, page_limit+1):
            url = url_flipkart + query_laptops + query_brand + brand + query_page + str(page)
            driver.get(url)
            ...
            for link in links:
                ...
                try:
                    ...
                    continue
    json.dump({"records": laptops}, jsonfile)
